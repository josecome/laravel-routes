...
Route::get('page_a/{slug}',  [PageController::class, 'Page']);

/* Dinamic Route*/
Route::get('/page_b/{slug}', [PageController::class, 'Page'])->name('page_a');

/* Dinamic Route that only accept letters as parameter */
Route::get('/page_c/{slug}',  [PageController::class, 'Page'])->where('slug', '[A-Za-z]+');

/* Dinamic Route that only accept number as parameter */
Route::get('/page_d/{id}',  [PageController::class, 'Page'])->where('id', '[0-9]+');

/* Option 1: Dinamic Route that only accept number as first parameter and letters as second parameter */
Route::get('/page_e/{id}/{slug}',  [PageController::class, 'Page'])->where(['id' => '[0-9]+', 'name' => '[a-z]+']);
...
Route::get('/page_f/{slug}',  [PageController::class, 'Page'])->whereAlphaNumeric('name');

/* Dinamic Route that accept UUID format parameter */
Route::get('/page_g/{id}',  [PageController::class, 'Page'])->whereUuid('id');

/* Dinamic Route with optional parameter */
Route::get('/page_h/{slug?}',  [PageController::class, 'Page'])->whereUuid('id');
